...
export async function GET(
  ...
  try {
    ...
    const { data, error } = await supabase
      .from('events')
      .select('id, name, start_date, end_date, share_url, creator_name, is_locked, calculated_date, created_at, password_hash')
    ...
    if (error) {
      ...
      return NextResponse.json(
        { data: null, error: 'Failed to fetch event' },
        { status: 500 }
      );
    }
